...
// Numbers can be compared using the standard numeric 
// comparison operators (eg. 5 > 4 and 4 <= 5) and 
...
// Explicit conversions: data could be lost and additional
// syntax in the form of a cast is required.
// As an int has less precision than a double,
// converting from an int to a double is safe and is thus an implicit conversion.
// However, converting from a double to an int could mean losing data,
// so that requires an explicit conversion.
}
